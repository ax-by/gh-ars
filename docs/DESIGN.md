...
**`scaleset.Client`는 Actions 서비스 호출 전부를 인스턴스 하나짜리 뮤텍스로 직렬화한다** (`client.go`의 `GetRunner`, `GetRunnerByName`, `GenerateJitRunnerConfig`, `RemoveRunner`, scale set 조회·생성 모두 왕복 전체에 `c.mu.Lock()`을 건다. v0.4.0 기준). 우리는 프로세스당 `*Client` 하나를 쓰므로 이 직렬화는 프로세스 전역이다. 결과 둘: (1) 같은 종류의 호출을 goroutine으로 흩어도 벽시계 시간이 줄지 않는다 — 뮤텍스에 줄을 선다. (2) 느린 호출 하나가 주 경로(`GenerateJitRunnerConfig`, `RemoveRunner`)를 그만큼 민다. 그래서 §8.3의 등록 대조는 회차를 겹쳐 띄우지 않는다. 예외는 long-poll `GetMessage`다: `MessageSessionClient`가 요청을 직접 만들어 이 뮤텍스를 잡지 않으므로 폴링은 막히지 않는다(`session_client.go`의 `getMessage`). 호출들은 `api.github.com`이 아니라 Actions 서비스(`actionsServiceURL`, 별도 admin JWT)로 가므로 REST 5,000/시간 예산의 대상이 아니고, GitHub이 이 엔드포인트의 한도를 공표하지 않으며 라이브러리에 429/`Retry-After` 처리도 없다. 실측은 E2E에서 응답 헤더로 확인한다.
...
| `msgResynced` | **부품은 스냅샷이 권위다.** `plan.Reconcile` 실행 **전에** 이 머신 소속 known unit의 `Parts`를 `Obs`가 보여준 부품 집합으로 덮어쓴다(정리 실패나 외부 rm으로 캐시가 어긋나면 §8.3의 slot 점유 계산이 틀어진다). `State`는 유지한다 — 상태 판정에는 GitHub 등록 대조가 필요해 `Reconcile`이 판정하지 않는다(§5 책임 경계). `Busy`도 유지한다 — 출처가 머신 이벤트가 아니라 메시지 세션(`msgJobStarted`)이라 SSH 단절로 낡지 않고, 되채우는 경로가 없어 리셋하면 남은 생애 동안 `false`로 고정된다. 그 다음 `Reconcile` 결과의 Adopt/RemoveUnit/RemoveOrphan을 적용한다(보류된 Dying 포함). RemoveOrphan은 unit id별로 모아 **runner 이름 없는 Dying unit**으로 등록한다(`Mode=sidecar`, `Foreign`, 관측된 Parts): `cleanupUnit`의 GitHub 단계만 건너뛰고 sidecar → 볼륨 → slice 순서, tick 재시도, slot 점유를 그대로 탄다(고아 등록은 GitHub이 자동 제거, §3.2). 관측 시각(`At`) 이후 `Starting`이 된 unit은 Creating 예외(§5). 부품 집합만 판정하고 GitHub은 조회하지 않는다. **순서가 규범이다**: ① Parts 덮어쓰기 + 이 머신 소속 `pendingCompletion` 비우기(§7.2-3 안전장치) → ② 예산 재계산(`applyInfoOnResync`)과 머신 healthy 복귀 — 정리·입양 명령을 보내려면 healthy 여야 하므로 ③보다 앞이다 → ③ `Reconcile` 적용(`markDying`이 새 항목을 넣는다). 비우기가 ③보다 **뒤**에 오면 §4.5가 열거한 "재동기화"라는 `markDying` 갱신 지점이 영원히 발동하지 않는 죽은 조항이 된다: 비우기는 단절 구간에 쌓인 낡은 항목을 터는 안전망이고 ②가 넣는 항목은 방금 스냅샷으로 확인한 새 사실이라, 뒤에 비우면 낡은 것을 털다가 새 것을 같이 버린다. 실패 방향도 뒤집힌다 — §7.2-3이 규정한 잔여 실패 모드는 "일시적 1개 과소 배치, 자가 치유"인데, 새 항목을 버리면 assigned에서 완료분이 빠지지 않아 반대로 유휴 runner가 생긴다. ②의 예산 재계산은 스냅샷의 `Info`로 한다(시작 시 미도달이었던 머신은 그때까지 0이라 복귀시키기만 해서는 capacity에 한 칸도 보태지 못한다). 이 경로의 R21 위반은 재판정이므로 `Failed`가 아니라 physicalMax 0 + 경고다(§7.1). |
| `msgTick` | **GitHub 등록 대조의 유일한 구현 주체.** Starting/Running unit(**Draining 제외**)을 모아 goroutine이 `GetRunner`를 부르고 결과를 `msgRegistration`으로 돌려보낸다(루프 안에서 GitHub을 부르지 않는다). **회차는 한 번에 하나만 돈다**: 앞 회차가 아직 돌고 있으면 새로 띄우지 않고 경고만 남긴다 — 그러면 그 회차가 문자 그대로 멈춘 지점부터 이어서 돈다. 겹쳐 띄우면 다음 tick에 비어 있는 unit은 **정확히 방금 대조를 마친 앞부분**이라(진행 중인 것은 비지 않는다) 앞부분만 반복해서 돌고 꼬리는 계속 밀린다 — grace 5분을 넘긴 `Starting` unit이 슬롯을 문 채 남는다. 호출을 흩어 벽시계 시간을 줄이는 길은 없다(§4.4 뮤텍스). 회차 하나의 unit 목록은 띄울 때의 스냅샷이며, 그 뒤에 생긴 unit은 다음 회차가 본다. `Creating`의 기동 타임아웃 판정. healthy 머신의 `Dying` unit 중 정리가 진행 중이 아닌 것은 `cleanupUnit` 재시도. `pendingCompletion`·`completedIDs`에서 5분 지난 항목 제거(§7.2-3 안전장치, §8.3 상수 표). [§8.3] |
...
Controller는 시작 시 머신마다 `Preflight`를 **goroutine으로 동시에** 부르고(§7.1-3), 결과를 모아 설정에 적힌 머신 순서로 적용한다 — `applyInfoAtStart`·`reached`·`Health`는 Controller 상태라 루프 goroutine만 만진다(§1-2), 로그 순서도 그래야 결정적이다. `ErrFatal`이 여러 머신에서 나오면 첫 건에서 멈추지 않고 전부 로그로 보인 뒤 `errors.Join`으로 올린다. pre-pull 상한은 이미지별이 아니라 **머신 하나의 pre-pull 단계 전체**에 건다(§8.3). 단 로컬 이미지 확인(`ImageExists`)은 그 상한 ctx가 아니라 **부모 ctx**로 돌린다: 상한 초과로 만료된 ctx로 확인하면 §7.1-4의 완화가 정확히 필요한 순간에 무력해진다.
